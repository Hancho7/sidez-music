      ...
      {collectionId && (
        <>
          ...
          <motion.aside
            ...
            className="fixed top-0 right-0 bottom-0 w-[480px] bg-surface border-l border-[color:var(--border-subtle)] z-[401] flex flex-col overflow-y-auto"
          >
            {loading || !detail ? (
              ...
              <>
                {/* Header */}
                <div className="px-6 pt-5 pb-0 border-b border-[color:var(--border-subtle)] flex-shrink-0">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {detail.isFeatured && <Star size={14} fill="#a855f7" className="text-accent flex-shrink-0" />}
                      </div>
                    ...
                    <div className="flex gap-2 flex-shrink-0">
                      <button
                        ...
                        className="px-3.5 py-1.5 bg-accent/15 text-accent border border-accent/30 rounded-lg cursor-pointer text-sm font-semibold hover:bg-accent/25 transition-colors"
                      >
                        Edit
                      </button>
                      <button
...
function TabBtn({ tab, active, onClick }: { tab: string; active: boolean; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-1.5 px-3.5 py-2 text-sm font-medium bg-transparent border-0 cursor-pointer capitalize transition-colors duration-150
        ${active
          ? "text-accent border-b-2 border-accent"
          : "text-[color:var(--text-muted)] border-b-2 border-transparent hover:text-foreground"
        }`}
    >
      {icons[tab]} {tab}
    </button>
...
/* ── Tracks Tab ── */
function TracksTab({
  tracks, onDragStart, onDragOver, onDrop, onRemove,
}: {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <button className="flex items-center gap-1.5 px-3 py-1.5 bg-accent/15 text-accent border border-accent/30 rounded-lg cursor-pointer text-xs font-semibold hover:bg-accent/25 transition-colors">
          <Plus size={13} /> Add Track
        </button>
      </div>

      {tracks.length === 0 ? (
        <div className="text-center py-10 text-[color:var(--text-muted)] text-sm">
        ...
        <div className="flex flex-col gap-1.5">
          {tracks.map((ct, i) => (
            <div
              ...
              onDrop={onDrop}
              className="flex items-center gap-2.5 px-3 py-2.5 bg-elevated border border-[color:var(--border-subtle)] rounded-lg cursor-grab transition-colors duration-150 hover:border-[color:var(--border-default)]"
            >
              <GripVertical size={14} className="text-[#31386d] flex-shrink-0" />
              <span className="text-xs text-[#31386d] w-[18px] text-center flex-shrink-0">{i + 1}</span>
              <img src={ct.track.coverImage} alt={ct.track.title} className="w-9 h-9 rounded-md object-cover flex-shrink-0" />
              <div className="flex-1 min-w-0">
              ...
              <span className="text-[11px] text-[color:var(--text-muted)] flex-shrink-0">{ct.track.duration}</span>
              <button
                onClick={() => onRemove(ct.trackId)}
                className="w-7 h-7 flex items-center justify-center bg-danger/10 border border-danger/20 rounded-md cursor-pointer text-danger hover:bg-danger/20 transition-colors flex-shrink-0"
              >
                <Trash2 size={12} />
              </button>
            </div>
...
function PerfCard({ icon, label, value, color }: { icon: React.ReactNode; label: string; value: string; color: string }) {
  return (
    <div className="bg-elevated border border-[color:var(--border-subtle)] rounded-xl p-4 flex flex-col gap-2.5">
  ...
  return (
    <div className="p-6">
      <div className="flex justify-end mb-5">
        <button onClick={onClose} className="bg-elevated border border-[color:var(--border-subtle)] rounded-lg w-8 h-8 flex items-center justify-center cursor-pointer text-[color:var(--text-muted)]">
          <X size={15} />
        </button>
      </div>
      {[180, 20, 12, 12, 12].map((h, i) => (
        <div key={i} className="animate-pulse" style={{ height: h, background: "#1a1f3a", borderRadius: 8, marginBottom: 14, width: i === 0 ? "100%" : `${90 - i * 10}%` }} />
      ))}
    </div>
  );
}
